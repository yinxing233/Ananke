...
- **阈值语义耦合**：persistence 阈值 3.0（对纯 IA 记忆需约 8 次激活）与 frequency 阈值 3（仅需 3 次）相差约 3 倍。两策略的 score 函数与阈值共同变化，单点数据无法分离"选择压力来源"与"阈值数值"各自的贡献。频率策略的语义本就是"不区分来源"，故差异是策略定义的一部分，非实验疏漏——但结论稳健性须由阈值 sweep 检验。
...
> **诚实边界（GLM 证伪审查后降级）**：本结果验证的是"两种不等价的迁移规则在同一输入流上产生可判别的结构差异，且分歧方向与理论预测一致"，**并非**"哪种结构更稳定"。协议 §5 真正的比较器（存活记忆 vs 演化中外部世界后期发展的一致性）与"稳定 vs 易被破坏"的标签**尚未测量**——本实验只测了"谁被升层"，从未做扰动/存活率检验。因此本发布称"Phase 3 先导实验（pilot）"，不称"Phase 3 证据"。阈值耦合（persistence 阈值 3.0 对纯 IA 记忆需约 8 次激活，frequency 阈值 3 仅需 3 次）是两种策略定义的一部分，单点数据无法分离"来源加权"与"阈值数值"的贡献；该耦合在下方已知限制中声明，稳健性留待阈值 sweep。
