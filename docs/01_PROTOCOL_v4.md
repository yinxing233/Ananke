...
2026-08-18 已在探索集 `conv-26` 完成 419 轮 P/F 双运行，仪器全链路已点火；但
EV 通道全程未激活，两项推测因缺少比较组而不可判，独立 judge 评测未执行。关系分类
验收、验证集锁定、预登记及其余 §8 条件仍未完成。不得把“探索性点火已完成”
叙述为“协议 v4 已正式冻结”或“C2 已被检验”。
...
**2026-08-18 空集降级条款（PI 批准）：**

- 推测 1 只有在 `P∖F` 与 `F∖P` 均非空，且两组都有有效 judge 分数时才可判。任一独有集为空，
  或任一组没有有效分数，结果必须输出 `not_testable`，不得用空集上的真空真值判为“成立”。
- 推测 2 只有在 `F∖P` 内 `EV=0` 与 `EV>0` 两个子组均非空，且两组都有有效 judge 分数时才可判。
  任一子组为空或缺少有效分数时，同样输出 `not_testable`。
- `not_testable` 不是“不成立”，也不是支持；它表示当次数据没有形成该推测所需的对照。增加 judge 请求
  不能修复缺失的组。
...
→ 余弦召回：对 WORKING + CONSOLIDATED + CORE 全部已有记忆检索
  cos ≥ R_recall 的候选对 [探索性点火沿用 R_recall=0.65；正式定值仍待独立校准]
...
- 机制签名检查：F\P 中 EV=0 记忆的富集度及其命中率。EV=0 在 persistence 中不被硬阻断，
  因而该签名不是定义上的必然（§4.2）。
- **可判性先于方向判定**：按 §0 的空集降级条款，推测 1 缺少任一独有集、或推测 2 缺少
  任一 EV 子组时，`divergence_analysis.py` 必须记 `not_testable`；不得以 `False`、`True`
  或“不成立/不可判”混合标签隐去结构性缺组。
...
- **探索阶段（当前）**：已使用 LoCoMo `conv-26` 完成 100 轮校准与 419 轮 P/F 双运行。
  允许调参、改协议、修 bug。本阶段所有数字标注“探索性”，不进入正式验证结论；
  `conv-26` 已暴露，不得再进入验证集。
- **冻结点**：v4 全部未定参数完成独立校准并填入定值、B1–B7 实现与守护测试通过 →
...
- [x] 已暴露 `conv-26` 的 100 轮真实校准与 419 轮 P/F 探索性全链路点火完成（2026-08-18）；此项不替代独立验收、验证集与预登记
- [x] 分歧分析空集降级已实现并由回归测试守护；全量测试 92 passed（2026-08-18）
- [ ] 轮级原子性与硬失败恢复冒烟通过
- [x] 探索性全链路冒烟通过（`conv-26` P/F 均完整结束、无 HTTP 错误或 429，提取量级与请求计量已入回执）；此项不等于关系分类器独立验收或 judge 评测
