...
状态：草案。冻结条件见 §8。
2026-07-28 已形成 B1–B7 **裁决冻结候选**（见
[`DECISIONS_v0.2_freeze.md`](./DECISIONS_v0.2_freeze.md)）；PI 验收与实现均未完成，
不得把该裁决集叙述为“协议 v4 已正式冻结”或“代码已经落地”。
...
v0.2 处理：冻结候选保持 top-1；冒烟时观察实际候选分布（多少 (m,e) 对存在多候选、矛盾对是否
...
- 计分冻结候选：包含=1.0，部分=0.5，不包含=0.0；标签必须互斥解析。
...
- [x] "部分包含"计分规则定值：0.5（B6 冻结候选；仍待实现测试）
- [ ] B1–B7 行为实现 + 守护测试通过
- [ ] 轮级原子性与硬失败恢复冒烟通过
