# Phase 1 实验协议 · v2（Experimental Protocol）

> ⚠️ **本版本已被 `01_PROTOCOL_v3.md` 取代（2026-07-14）。** 保留供审计。v3 变更：EV 阈值 0.85→0.80；新增「写入前去重」控制变量（DEDUP_SIMILARITY_THRESHOLD=0.80）；提取 Prompt 改为「输出与输入同语言」。

> **版本**：v2（2026-07-14 由 v1 升级）。变更：EV 与 IA 从「重叠」改为「互斥」（PI 决策）。原 v1 形式化定义 `NOT EV` 与重叠声明自相矛盾，统一为互斥方向。v1/v2 保留于 `01_PROTOCOL_v1.md` / `01_PROTOCOL_v2.md` 供审计。
...
- ⚠️ **互斥声明（冻结）**：对同一条记忆 m 和同一条输入 x，EV 和 IA 互斥。判定顺序：先判 EV(m, x)，若为 True 则 IA(m, x) 不再判定。每条记忆每轮最多触发一个信号。理由：EV（环境独立确认）与 IA（系统内部关联发现）是不同性质的事件；cosine ≥ 0.85 必然 ≥ 0.60 是检测机制的数学性质，不是事件具有双重理论性质。互斥保证 persistence_score 可精确归因（外部压力 vs 内部压力），且阈值 3.0 的语义不变（≈ 3 次纯外部验证）。
