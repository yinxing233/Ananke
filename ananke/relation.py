...
_LLM_SYSTEM_PROMPT = (
    ...
    "Decide in this strict order, using ONLY the two sentences given:\n"
    "1. SAME FACT? If the new memory restates the existing fact (mutually entailing; deleting either "
    "sentence loses no information) -> duplicate.\n"
    "2. SAME FRAME? If both sentences refer to the same event / plan / attitude slot but differ in details:\n"
    "   - compatible additions -> mergeable\n"
    "   - incompatible (cannot both hold) -> contradict\n"
    "3. SAME TOPIC? Otherwise, if the two sentences share a clear concrete topic (enough to explain why "
    "they would be recalled together) but are NOT the same fact or frame -> related.\n"
    "   If they share no clear concrete topic (only a shared person name, or only vague positive tone, "
    "or different topics) -> unrelated.\n"
    "Cross-sentence rules:\n"
    "- A general statement and a specific instance may be merged ONLY if both are the same modality "
    "(both plans, both events, or both attitudes); across modalities at most related.\n"
    "- For attitude memories, mergeable requires the SAME attitude predicate (e.g. admires+admires "
    "may merge; inspired+admires cannot).\n"
    "- Do NOT infer hidden causality or coreference not written in the two sentences; praising vs. "
    "thanking are independent speech acts.\n"
